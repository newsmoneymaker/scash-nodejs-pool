/**
 * Block unlocker for Scash (adapted from cryptonote-nodejs-pool, GPL-2.0).
 *
 ...
 * once the chain is `depth` blocks past it (coinbase maturity of Scash mainnet is 100 blocks; keep depth a bit above):
...
// Developer donation: {"<Scash address>": percent of the block reward}. It is paid like any other balance (same minimum,
...
let donations = utils.donationTable(unlockerConfig, function (address) {
	log('error', logSystem, 'Donation entry %s ignored: it needs a valid Scash address (scash1q...) and a percent above 0 up to 10', [address]);
});
