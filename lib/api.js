...
let scashReward = require('./reward.js');
let rpc = require('./scashRpc.js');
...
function lookupAddress (input) {
	let value = input.trim().replace(/^(prop|solo):/i, '');
	value = value.split(/[+\s]/)[0];            // drop the worker name
	return utils.canonicalMinerAccount(value) || input;      // KEY@domain, or KEY@domain#NOTE when a deposit note is given
}
...
function handleServerRequest (request, response) {
	...
	// Statistics are kept per canonical address (KEY@domain). People paste what they type into the miner instead:
	// "prop:KEY@domain+rig1", a bare key, with spaces. Accept all of that.
	if (typeof urlParts.query.address === 'string') {
		urlParts.query.address = lookupAddress(urlParts.query.address);
	}
...
function collectStats () {
	...
	async.parallel({
		...
		config: function (callback) {
			let unlocker = config.blockUnlocker || {};
			...
			callback(null, {
				...
				fee: unlocker.poolFee || 0,
				soloFee: unlocker.soloFee >= 0 ? unlocker.soloFee : (unlocker.poolFee > 0 ? unlocker.poolFee : 0),
				networkFee: unlocker.networkFee || 0,
				// The website uses this as an HTML id and as the key of everything it draws per coin (it has to match
				// parentCoin in the website's config.js): the ticker, not the name with a space that keys the Redis data.
				coin: config.symbol,
				coinUnits: config.coinUnits,
				coinDecimalPlaces: config.coinDecimalPlaces || 8,
				coinDifficultyTarget: config.coinDifficultyTarget,
				symbol: config.symbol,
				depth: unlocker.depth,
				finderReward: unlocker.finderReward || 0,
				donation: utils.donationTable(config.blockUnlocker),
				version: version,
				paymentsInterval: payments.interval,
				minPaymentThreshold: payments.minPayment,
				maxPaymentThreshold: payments.maxPayment || null,
				transferFee: payments.transferFee || 0,
				denominationUnit: payments.denomination,
				slushMiningEnabled: !!(config.poolServer.slushMining && config.poolServer.slushMining.enabled),
				weight: config.poolServer.slushMining ? config.poolServer.slushMining.weight : null,
				priceSource: config.prices ? config.prices.source : null,
				priceCurrency: config.prices ? config.prices.currency : null,
				paymentIdSeparator: '.',
				fixedDiffEnabled: false,
				fixedDiffSeparator: '.',
				sendEmails: false,
				blocksChartEnabled: !!(config.charts && config.charts.blocks && config.charts.blocks.enabled),
				blocksChartDays: config.charts && config.charts.blocks && config.charts.blocks.days ? config.charts.blocks.days : null,
				telegramBotName: null,
				// Scash specific
				workerSeparator: '+',
				algorithms: ['randomx'],
				blockMaturity: (config.blockUnlocker && config.blockUnlocker.depth) || 100,
				// one full block = blockScale units of pool.roundHashes (see shares.js)
				blockScale: require('./shares.js').BLOCK_SCALE
			});
		},
...
/**
 * Get Network data: height of the block being mined, and the block difficulty of each
 * algorithm as last seen by the pool in a node job template (published to redis by
 * pool.js). The headline `difficulty` is the one of the algorithm being mined.
 **/
function getNetworkData (callback) {
	rpc.call('getblockcount', [], function (error, blocks) {
		if (error || typeof blocks !== 'number') {
			log('error', logSystem, 'Error getting network data %j', [error ? (error.message || JSON.stringify(error)) : 'bad reply']);
			callback(true);
			return;
		}

		redisClient.hgetall(config.coin + ':network', function (redisError, network) {
			network = network || {};
			let difficulties = {};
			try {
				difficulties = JSON.parse(network.difficulties || '{}');
			} catch (e) {}

			let algorithm = network.algorithm || null;
			callback(null, {
				difficulty: (algorithm && difficulties[algorithm]) || 0,
				height: blocks + 1,
				algorithm: algorithm,
...
/**
 * Get Last Block data (chain tip). The reward shown is the one the consensus rules give to miners at this height
 * (no fees); the pool reads the exact reward of its own blocks from its wallet.
 **/
function getLastBlockData (callback) {
	rpc.call('getbestblockhash', [], function (error, hash) {
		if (error || typeof hash !== 'string') {
			log('error', logSystem, 'Error getting last block data %j', [error ? (error.message || JSON.stringify(error)) : 'bad reply']);
			callback(true);
			return;
		}
		rpc.call('getblockheader', [hash], function (error, header) {
			if (error || !header || !header.hash) {
				log('error', logSystem, 'Error getting last block header %j', [error ? (error.message || JSON.stringify(error)) : 'bad reply']);
				callback(true);
				return;
			}
			callback(null, {
				...
				reward: scashReward.minerReward(header.height),
				hash: header.hash
			});
		});
	});
		...
		redisClient.exists(`${config.coin}:workers:${address}`, function (error, result) {
			if (!result) {
				response.end(JSON.stringify({
					error: 'No shares from this address yet. Enter the address your miner logs in with. / С этого адреса ещё не было шар. Введите адрес, под которым входит майнер.'
				}));
				return;
			}
 ...
 **/
/**
 * Shorten a Scash address for the public top 10: the beginning and the end (every address starts with "scash1q").
 **/
function shortMinerAddress (account) {
	let address = utils.splitMinerAccount(account).address;
	return address.length > 16 ? address.substring(0, 8) + '...' + address.substring(address.length - 6) : address;
}
		...
		function (workerKeys, callback) {
			...
			redisClient.multi(redisCommands).exec(function (error, redisData) {
					...
					for (let i in redisData) {
						...
						data = redisData[i];
						minersData.push({
							miner: shortMinerAddress(address),
