 ...
 *
 * Stratum server for Scash's RandomX ("rx/scash", XMRig style JSON-RPC over TCP/TLS).
 *
 *   node getblocktemplate --> block builder (coinbase, merkle root, 112 byte header) --> job manager --job--> miners
 *   miners --submit--> share check (helper: RandomX hash R and commitment CM of the header) --> redis (shares.js)
 *   a share that meets the network target is sent to the node as a whole block with submitblock.
 *
 * Scash's proof of work (doc/scash-protocol-spec): the RandomX key is SHA256d("Scash/RandomX/Epoch/<time / 7 days>"), the input is the
 * 112 byte header with hashRandomX zero (nonce = 4 bytes at offset 76), R = RandomX(header); the block is valid when the RandomX
 * commitment CM = commitment(header, R), read as a little endian number, is not above the target, and the header carries R.
 * Job target: 64 bit, little endian hex, the miner compares the last 8 bytes of CM (little endian) with it.
 **/
...
let COINBASE_TAG = poolConfig.coinbaseTag === undefined ? '/scash.pool-pay.com/' : poolConfig.coinbaseTag;
...
const NONCE_OFFSET_HEX = 152;      // byte 76 of the header
const HEADER_HEX_LENGTH = 224;     // 112 bytes
...
/**
 * Parse the stratum login: [prop:|solo:]<scash1q address>[+workername]
...
function parseLogin (login, pass) {
	...
	if (!address) return {error: 'Invalid Scash address (expected a native SegWit address: scash1q..., 45 characters)'};
...
/** the RandomX key of the epoch a block time falls into: SHA256d("Scash/RandomX/Epoch/<epoch>") */
function epochKey (time, duration) {
	let epoch = Math.floor(time / duration);
	return builder.sha256d(Buffer.from('Scash/RandomX/Epoch/' + epoch)).toString('hex');
}

function newJobFromTemplate (t) {
	if (!poolScript) {
		poolScript = utils.addressScript(poolConfig.poolAddress);
		if (!poolScript) throw new Error('poolServer.poolAddress is not a valid scash1q... address');
	}
	if (!t || !t.previousblockhash || !t.bits || !t.target || typeof t.coinbasevalue !== 'number') throw new Error('unexpected block template');
	let extra = Buffer.alloc(8);
	crypto.randomBytes(4).copy(extra, 0);
	extra.writeUInt32BE((jobCounter + 1) >>> 0, 4);
	let built = builder.buildJob(t, poolScript, extra, COINBASE_TAG);
	if (built.target <= 0n) throw new Error('bad target ' + t.target);
	return {
		id: null,
		height: t.height,
		header: built.header.toString('hex'),
		built: built,
		keyRaw: epochKey(t.curtime, t.rx_epoch_duration || 7 * 24 * 60 * 60),
		target: built.target,
		blockHashes: Number(TWO_256 / (built.target + 1n)),
		created: Date.now(),
	};
}
	...
	rpc.call('getbestblockhash', [], function (err, tip) {
		...
		rpc.call('getblocktemplate', [{rules: ['segwit']}], function (err, t) {
			...
			if (job.keyRaw !== currentKeyRaw) {
				// a new RandomX key (every 7 days): the helper rebuilds its dataset, jobs start when it is ready
...
function handleConnection (socket, portData) {
	...
	function onSubmit (msg) {
		...
		hasher.hash(blob, function (err, hashes) {
			...
			// the miner's result is the RandomX commitment (the value the block is judged by)
			if (params.result && String(params.result).toLowerCase() !== hashes.cm) {
				return reject(msg, 'Bad hash');
			}
			let value = Buffer.from(hashes.cm, 'hex').readBigUInt64LE(24);
			...
			let isBlock = BigInt('0x' + reverseHex(hashes.cm)) <= job.target;
			sent.nonces.add(nonce);
			checkBan(miner, true);
			reply(msg.id, {status: 'OK'});
			acceptShare(job, credited, nonce, hashes.r, isBlock);
		});
	}
	...
	function acceptShare (job, sent, nonce, hashHex, isBlock) {
		...
		// full solution: the node takes the whole block (header with the nonce and the RandomX hash, coinbase, transactions)
		log('info', logSystem, 'Block solution at height %d from %s (worker %s), submitting', [job.height, miner.address, miner.workerName || '-']);
		let blockHex = builder.serializeBlock(job.built, nonce, hashHex);
		let blockHeader = Buffer.from(blockHex.substr(0, HEADER_HEX_LENGTH), 'hex');
		let blockHash = reverseHex(builder.sha256d(blockHeader).toString('hex'));
