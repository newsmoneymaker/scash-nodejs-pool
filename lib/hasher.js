 ...
 *
 * Wrapper around the rxhash helper (hasher/rxhash.cpp): RandomX hash and commitment of the block header (--scash) for share validation.
 * Settings in config.hasher: {path, threads, light, args (default ["--scash"])}
 *   setKey(keyHex, cb)   the RandomX key (raw bytes), the process re-initialises its dataset (10-60 s)
 *   hash(blobHex, cb)    cb(err, {r, cm}): the RandomX hash and the RandomX commitment of the header (32 raw bytes each, hex)
...
function start () {
	let file = hasherConfig.path || path.join(__dirname, '..', 'hasher', 'rxhash');
	let args = (hasherConfig.args || ['--scash']).concat(['--threads', String(hasherConfig.threads || 2)]);
	...
	child.on('exit', function (code, signal) {
		log_('error', 'rxhash exited (code %s, signal %s), restarting in 3 s', [code, signal]);
		child = null;
		ready = false;
...
function onLine (line) {
	...
	if (parts[0] === 'K') {
		...
		if (parts[2] === 'err') cb(new Error('hasher error')); else cb(null, {r: parts[2], cm: parts[3]});
	}
}
