// rxhash: share validation helper of the Scash pool.
...
//   H <id> <hex blob>     with --scash: R = RandomX(blob) and CM = randomx_calculate_commitment(blob, R); answers "H <id> <R hex> <CM hex>"
//                         with --raw the blob is hashed as it is: hash = RandomX(blob) (Epic Cash)
//   Q                     quit
// Scash's proof of work: RandomX (with the Scash Argon2 salt, compiled into the node's library) over the 112 byte header; the block is
// judged by the RandomX commitment of the header and the hash.
#include <randomx.h>
#include <atomic>
...
static bool rawMode = false;      // --raw: RandomX over the blob itself
static bool scashMode = false;    // --scash: RandomX over the blob, answer with the hash and the commitment
...
static void worker(size_t idx) {
	for (;;) {
		...
		unsigned char cm[RANDOMX_HASH_SIZE];
		if (scashMode) {
			randomx_calculate_hash(vms[idx], t.blob.data(), t.blob.size(), out);
			randomx_calculate_commitment(t.blob.data(), t.blob.size(), out, cm);
		} else if (rawMode) {
			randomx_calculate_hash(vms[idx], t.blob.data(), t.blob.size(), out);
		} else {
			randomx_calculate_hash(vms[idx], t.blob.data(), t.blob.size(), out);
		}
		say("H " + t.id + " " + toHex(out, RANDOMX_HASH_SIZE) + (scashMode ? " " + toHex(cm, RANDOMX_HASH_SIZE) : ""));
		{
...
int main(int argc, char **argv) {
	...
	for (int i = 1; i < argc; i++) {
		...
		else if (!strcmp(argv[i], "--raw")) rawMode = true;
		else if (!strcmp(argv[i], "--scash")) scashMode = true;
